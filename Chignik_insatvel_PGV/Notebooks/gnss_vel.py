...
infile = 'Chignik_phase_vels/ac28_1hz.L2.igr.phase_vels'
...
st_gnss_vz[0].stats.station = 'AC28'
...
st_gnss_vz[0].trim(dt+10, dt+170)
#st_gnss_vz[0].plot()
...
st_seis_az[0].filter('bandpass', freqmin=0.01, freqmax=0.2, corners=4, zerophase=True)
...
st_sm_vz[0].stats.starttime = UTCDateTime("2021-07-29T06:15:59.0Z")
...
axx[2, 1].plot(lags, cc, 'k', linewidth=1, label='AC28*CHN (1Hz)')
...
axx[1, 1].set_xlabel('time 10s after origin-time (s)')
#axx[0, 0].set_xlabel('time 20s after origin-time (s)')
axx[2, 0].set_xlabel('time 10s after origin-time (s)')
