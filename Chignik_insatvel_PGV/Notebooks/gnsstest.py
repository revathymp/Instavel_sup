...
dt = UTCDateTime("2021-07-29T06:15:49")
st_gnss_vz[0].trim(dt-20, dt+180)
...
#st_seis_az_d = st_seis_az.copy()
#st_seis_az_d.decimate(factor=16, strict_length=False)
#st_seis_az.filter('lowpass', freq=0.499, corners=2, zerophase=True)
#st_seis_az_d.filter('lowpass', freq=0.499, corners=2, zerophase=True)
st_seis_az.filter('bandpass', freqmin=0.01, freqmax=0.499, corners=4, zerophase=True)

# CREATE A SEISMIC VELOCITY TRACE
st_seis_vz = st_seis_az.copy()
st_seis_vz.integrate(method='cumtrapz')

#st_seis_vz_d = st_seis_az_d.copy()
#st_seis_vz_d.integrate(method='cumtrapz')

# NORMALIZE EACH TRACE WITH ITS ABSOLUTE MAXIMUM
st_seis_az.normalize()
st_seis_vz.normalize()
#st_seis_vz_d.normalize()
...
st.plot(outfile="AB13-S15K-E-BP-Filt.pdf", format="pdf", equal_scale=True, automerge=False, ticks="5")
